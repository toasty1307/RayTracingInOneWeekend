...
internal class Program
{
    ...
    public const int ImageWidth = 1920; // 3840
    ...
    private void ActualMain()
    {
        ...
        if (ThreadCount == -1)
            ThreadCount = Math.Min(ImageHeight, workerThreads);
...
#pragma warning disable CA1416
        ...
        var rowsForOneThread = ImageHeight / ThreadCount;
        var start = ImageHeight - 1;
        ...
        Log.Information("Started all threads!");
        /*
        for (var j = ImageHeight - 1; j >= 0; j--)
        {
            ...
            for (var i = 0; i < ImageWidth; i++)
            {
                ...
                WriteColor(_image, colorVector, i, j);
            }
        }
        */
        ...
        try
        {
            ...
            for (var i = 0; i < ThreadCount; i++)
            {
                bitmaps[i] = new Bitmap($"output{i}.png");
            }
        
            var finalImage = MergedBitmaps(bitmaps);
            finalImage.RotateFlip(RotateFlipType.Rotate180FlipX);
            finalImage.Save("finaloutput.png");
        
            Log.Information("Images combined and saved!");

            Process.Start(new ProcessStartInfo("finaloutput.png") { UseShellExecute = true });
            Console.ReadLine();
        }
    ...
    private Bitmap MergedBitmaps(IEnumerable<Bitmap> bitmaps) 
    {
